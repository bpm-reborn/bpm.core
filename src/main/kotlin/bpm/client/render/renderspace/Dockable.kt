...
abstract class Dockable(val name: String, protected val parent: Dockable? = null) {
     ...
     */
    fun process(gfx: GuiGraphics, graphics: CanvasGraphics) {
        render(gfx, graphics)
     ...
     * @since 1.0.0
     */
    protected open fun render(gfx: GuiGraphics, graphics: CanvasGraphics) {
        dockedWindows.forEach { (_, window) ->
            window.render(gfx, graphics)
        }
    ...
    /**
     * Uses the split direction and the percentage of space we currently occupy in the parent to calculate the size of the split.
     *
     * If the parent is null, we assume the split is 100% of the space.
     *
     * @param oldSize the old size of the viewport
     * @param newSize the new size of the viewport
     */
    fun onResize(oldSize: Vector2f, newSize: Vector2f) {

//        if (width == 0 || height == 0) return
//        // Calculate percentages of viewport currently occupied
//        val widthPercent = width / newSize.x
//        val heightPercent = height / newSize.y
//        // Calculate new absolute size based on the percentage and new viewport size
//        split.size = when (split.direction) {
//            Split.Direction.LEFT, Split.Direction.RIGHT -> widthPercent
//            Split.Direction.UP, Split.Direction.DOWN ->  heightPercent
//            Split.Direction.CENTER -> 1.0f
//        }
//
//        markDirty()
//
...
//        dockedWindows.values.forEach { window ->
//            window.onResize(oldSize, newSize)
//        }
//        floatingWindows.forEach { window ->
//            window.onResize(oldSize, newSize)
//        }
